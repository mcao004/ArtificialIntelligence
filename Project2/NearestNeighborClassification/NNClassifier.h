...
class NNClassifier {
    public:
        ...
        float classify(const vector<Instance> &training,const Instance &newData,const vector<int> &subset);        
        // returns the Euclidean distance of the newData from the training data
        float dist(const Instance &i1,const Instance &i2,const vector<int> &subset);
};
#endif
